...
```{r}
priming <- read.table("Moussavi/priming.tab",header = T)
...
```{r}
priming.f1.item <- ezANOVA(priming
                      , dv=.(RT)
                      , wid=.(item)
                      , within=.(prime,target)
                      , detailed=TRUE
...
```{r, results='asis'}
print(xtable(priming.f1.item$ANOVA),type="html",include.rownames=FALSE)
```

print(xtable(priming.f1.item$ANOVA
             , display=c("s","s","d","d","f","f","f","fg","s","g")
             , digits=c(0,0,0,0,2,2,2,2,0,2))
      ,type="html",include.rownames=FALSE)
```
