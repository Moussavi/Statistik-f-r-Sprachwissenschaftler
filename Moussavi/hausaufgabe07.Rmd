...
```{r}
for(s in c(3,4,5) ){
  sehr.gut <- pnorm(12,mean=mu,sd=s, lower.tail = FALSE)
  output <- paste("Bei einer Standabweichung von",s, "haben",sehr.gut*100,"% sehr gute Noten.")
  print(output)
}
```
Wie steht die Anzahl guter Noten in Beziehung zur Anzahl schlechter Noten? 

Die Anzahl guter Noten ist geringer als die Anzahl schlechter Noten (teilweise sogar nur ein Drittel der Zahl schlechter Noten.

Warum?

Durch die ungleiche Anzahl an Noten, mit denen man im sehr guten Bereich liegt (13, 14, 15 Punkte) bzw. durchgefallen ist (0, 1, 2, 3, 4, 5 Punkte), steigt auch die Wahrscheinlichkeit einer schlechten Note. Auch durch eine hohe Standardabweichung wird dies begünstigt.
...
2. Wie viele Studenten bekommen zumindest 10 NP?
    ...
    `r (pnorm(9,mean=mu,sd=3, lower.tail = FALSE)) * n` Studenten bekommen zumindest 10 Notenpunkte.

3. Wie viele Studenten bekommen weniger als 10 NP?

     `r (pnorm(9,mean=mu,sd=3)) * n` Studenten bekommen weniger als 10 Notenpunkte.

4. Wie viele Studenten bekommen weniger als 8 NP?

     `r (pnorm(7,mean=mu,sd=3)) * n` Studenten bekommen weniger als 8 Notenpunkte.
...
Bei diesem Dozenten ist die Verwaltung wirklich unsicher, ob und was für einen Unterschied es geben könnte. (Welche Testart sollte man hier nutzen?) Zweiseitiger Test, da Abweichungsgründe noch nicht klar.
...
```{r}
z.Wert.50 <-sqrt(50)*((10-8)/3)
print(z.Wert.50)
```

Das ist ein signifikanter Unterschied. 

Ist die Benotung vom Dozenten weniger als ideal? Ja, der Unterschied zum kritischen Wert ist hier zu groß.

### Ein anderer, böserer? Dozent
Die Verwaltung ist auch auf einen anderen Dozenten aufmerksam geworden, weil manche Studenten behaupten, er würde zu streng benoten. (Welche Testart sollte man hier nutzen?) Einseitiger Test, da vor allem die schlechteren Noten betrachtet werden müssen.
